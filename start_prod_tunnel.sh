...
generate_env_vars() {
    ...
    # Generate secret key base if not provided or if it's too short
    if [ -z "$SECRET_KEY_BASE" ] || [ ${#SECRET_KEY_BASE} -lt 64 ]; then
        print_info "Generating new SECRET_KEY_BASE (64+ characters)..."
        SECRET_KEY_BASE=$(mix phx.gen.secret)

        # Verify the generated key is long enough
        if [ ${#SECRET_KEY_BASE} -lt 64 ]; then
            print_error "Generated SECRET_KEY_BASE is too short (${#SECRET_KEY_BASE} chars). Expected at least 64."
            exit 1
        fi

        print_info "Generated SECRET_KEY_BASE: ${#SECRET_KEY_BASE} characters"
    else
        print_info "Using existing SECRET_KEY_BASE: ${#SECRET_KEY_BASE} characters"
    fi

    # Force set production database URL (override any existing value)
...
# Function to update Cloudflare config with dynamic port
update_cloudflare_config() {
    local port=$1
    local config_file="$HOME/.cloudflared/config.yml"

    if [ -f "$config_file" ]; then
        print_info "Updating Cloudflare config with port $port..."

        # Create a backup
        cp "$config_file" "$config_file.backup"

        # Update all service URLs to use the dynamic port
        sed -i.tmp "s|service: http://localhost:[0-9]*|service: http://localhost:$port|g" "$config_file"
        rm -f "$config_file.tmp"

        print_success "Updated Cloudflare config to use port $port"
    fi
}
...
start_cloudflare_tunnel() {
    ...
    if [ -f "$HOME/.cloudflared/config.yml" ] && [ ! -z "$CLOUDFLARE_TUNNEL_NAME" ]; then
        print_info "Using named tunnel: $CLOUDFLARE_TUNNEL_NAME"

        # Update config file with dynamic port
        update_cloudflare_config $port

        print_info "Config file: $HOME/.cloudflared/config.yml"
        # Start named tunnel with config file
        cloudflared tunnel run "$CLOUDFLARE_TUNNEL_NAME" > tunnel.log 2>&1 &
        TUNNEL_PID=$!
        TUNNEL_TYPE="named"
    else
        print_info "Using quick tunnel (trycloudflare.com)"
        print_info "For multi-subdomain setup, see: docs/CLOUDFLARE_MULTI_SUBDOMAIN.md"
    ...
    if kill -0 $TUNNEL_PID 2>/dev/null; then
        ...
        if [ "$TUNNEL_TYPE" = "quick" ]; then
            # Try to extract the tunnel URL from cloudflared logs for quick tunnels
            if [ -f "tunnel.log" ]; then
                TUNNEL_URL=$(grep -o 'https://.*\.trycloudflare\.com' tunnel.log | head -1)
                if [ ! -z "$TUNNEL_URL" ]; then
                    print_success "Quick Tunnel URL: $TUNNEL_URL"
                else
                    print_info "Check tunnel.log for the exact tunnel URL"
                fi
            fi
        else
            print_success "Named tunnel '$CLOUDFLARE_TUNNEL_NAME' is running"
            print_info "Your subdomains should be accessible via your configured domains"
            print_info "Check Cloudflare dashboard for tunnel status and traffic"
        fi
        return 0
    else
...
display_info() {
    ...
    # Show tunnel info based on type
    if [ "$TUNNEL_TYPE" = "quick" ]; then
        # Show tunnel URL if available for quick tunnels
        if [ -f "tunnel.log" ]; then
            TUNNEL_URL=$(grep -o 'https://.*\.trycloudflare\.com' tunnel.log | head -1)
            if [ ! -z "$TUNNEL_URL" ]; then
        ...
        echo -e "${GREEN}Tunnel Type:${NC} Named tunnel ($CLOUDFLARE_TUNNEL_NAME)"
        echo -e "${GREEN}Config:${NC} ~/.cloudflared/config.yml"
    fi
    ...
    if [ "$TUNNEL_TYPE" = "quick" ]; then
        print_info "Using quick tunnel - for multi-subdomain setup see: docs/CLOUDFLARE_MULTI_SUBDOMAIN.md"
    else
        print_info "Using named tunnel with multi-subdomain support"
    fi
...
main() {
    ...
    if [ ! -z "$CLOUDFLARE_TUNNEL_NAME" ]; then
        print_info "Named tunnel specified: $CLOUDFLARE_TUNNEL_NAME"
    fi
