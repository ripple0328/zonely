defmodule Zonely.PronunceName.Providers.NameShouts do
  ...
  def fetch(name, language) do
    ...
    if !api_key do
      ...
      url_with_lang =
        "https://www.v1.nameshouts.com/api/names/#{URI.encode(name)}/#{URI.encode(lang_name)}"
      ...
      Logger.info(
        "🌐 NameShouts request for #{inspect(name)} pref_without_lang=#{prefer_without_lang_first} lang=#{lang_name}"
      )

      case (prefer_without_lang_first && PronunceName.http_client().get(url_without_lang, headers)) ||
             PronunceName.http_client().get(url_with_lang, headers) do
        ...
        {:ok, %{status: 404}} ->
          alt_resp =
            if prefer_without_lang_first,
              do: PronunceName.http_client().get(url_with_lang, headers),
              else: PronunceName.http_client().get(url_without_lang, headers)
          ...
          case alt_resp do
            ...
            {:error, %Jason.DecodeError{data: data2}} ->
              Logger.warning(
                "NameShouts returned malformed JSON on alt route; attempting recovery"
              )
