  ...
  defp handle_nameshouts_response(body, requested_name, original_name, language) do
    case PronunceName.pick_nameshouts_variant(body, requested_name, language) do
      {:ok, paths} when is_list(paths) ->
        Logger.info("🔗 NameShouts returned multiple paths (sequential playback): #{inspect(paths)}")
        # Return direct URLs for sequential playback instead of concatenation
        urls = Enum.map(paths, fn path ->
      ...
      {:ok, path} when is_binary(path) ->
        ...
        if name_has_multiple_parts and requested_name == original_name and actual_name_part != original_name and actual_is_last and not actual_is_first do
          Logger.info("🚫 Rejecting last-name-only result ('#{actual_name_part}') for full name '#{original_name}'")
          {:error, :partial_only}
        else
