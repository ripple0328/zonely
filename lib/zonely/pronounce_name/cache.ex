defmodule Zonely.PronunceName.Cache do
  ...
  def lookup_cached_audio(name, language) do
    ...
    with {:ok, entries} <- File.ls(cache_dir) do
      ...
      # Generate the expected polly hash for this name/language combination
      polly_voice = Zonely.PronunceName.pick_polly_voice(language)
      polly_key = :crypto.hash(:sha256, Enum.join([name, language, polly_voice], ":"))
      ...
      # Prefer regular files (real person) over Polly files (AI)
      all_files = regular_files ++ polly_files
