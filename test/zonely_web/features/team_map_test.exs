defmodule ZonelyWeb.Features.TeamMapTest do
  ...
  describe "Team Map - Core Navigation" do
    feature "user can navigate to map page and see basic layout", %{session: session} do
      ...
      |> assert_has(css(".fixed.left-0.top-16"))  # Map takes full viewport
    end

    @tag :skip # TODO: Fix intermittent nav click handling under LV; re-enable after adding test helpers
  ...
  describe "Team Map - User Interaction" do
    ...
    @tag :skip # TODO: Wire marker clicks to LV event; expose data-testid on marker for targeting
    feature "user can view profile modal by clicking on map marker", %{session: session} do
      ...
      |> execute_script("document.querySelector('#map-container').dispatchEvent(new CustomEvent('click'))")
      |> assert_has(testid("team-map"))
      |> assert_text("Alice Johnson")
      |> assert_text("Frontend Developer")
    end

    @tag :skip # TODO: Ensure profile modal is opened before close assertion; depends on previous test
    feature "user can close profile modal", %{session: session} do
      session
      |> visit("/")
      |> wait_for_liveview()
      |> assert_has(testid("team-map"))
      |> click(testid("profile-modal"))  # Click overlay to close
  ...
  describe "Team Map - Audio Features" do
    setup do
      ...
      user_english_only = create_test_user(%{
        id: 5,
        name: "John Doe",
        country: "US",
    ...
    @tag :skip # TODO: Event wiring for pronunciation buttons inside profile modal; ensure modal opens
    ...
    @tag :skip # TODO: Ensure native button availability and modal; event wiring
    feature "user can play native pronunciation when available", %{session: session} do
      ...
      |> mock_audio_support()
      |> assert_has(testid("team-map"))
      |> assert_has(testid("pronunciation-native"))  # Should be visible for Spanish user
      |> click(testid("pronunciation-native"))
    end

    @tag :skip # TODO: Ensure modal; assert absence when name_native is nil or same
    feature "native pronunciation is hidden for English-only users", %{session: session} do
      session
      |> visit("/")
      |> wait_for_liveview()
      |> assert_has(testid("team-map"))
      |> refute_has(testid("pronunciation-native"))  # Should not be visible
      |> assert_has(testid("pronunciation-english"))  # But English should be
    end
  end
  ...
  describe "Team Map - Quick Actions" do
    ...
    @tag :skip # TODO: Requires open modal; stabilize quick actions rendering
    ...
    @tag :skip # TODO: Requires open modal; stabilize flash assertions
    feature "user can trigger quick message action", %{session: session} do
      ...
      |> assert_has(css(".flash-info"))  # Should show success flash
  ...
  describe "Team Map - Working Hours Overlap" do
    ...
    @tag :skip # TODO: Expose data-testid for the panel content and selector; stabilize click
    ...
    @tag :skip # TODO: Expose data-testid for time selector; stabilize interactions in tests
    feature "user can interact with time scrubber", %{session: session} do
      session
      |> visit("/")
      |> wait_for_liveview()
      |> click(testid("panel-toggle"))  # Expand panel
      |> assert_has(testid("time-range-selector"))
      |> assert_has(css("#time-scrubber"))
      |> drag_time_scrubber(0.375, 0.625)  # 9 AM to 3 PM
      |> assert_has(css("#scrubber-selection"))
    end

    @tag :skip # TODO: Expose data-testid and wire overlap_update to assert; later re-enable
    feature "time scrubber shows user availability status", %{session: session} do
      session
      |> visit("/")
      |> wait_for_liveview()
      |> click(testid("panel-toggle"))
      |> drag_time_scrubber(0.375, 0.5)  # 9 AM to 12 PM
