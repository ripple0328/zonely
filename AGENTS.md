...
- Before reshaping reachability, scheduling, overlap, holiday, leave, preference, calendar, map projection, or API boundary logic, read `docs/foundational-capabilities.md`. Zonely's durable foundation is a composable availability and reachability engine; the map and LiveView surfaces are projections on top of that foundation.
- When adding foundational behavior, prefer small composable primitives that are independently useful, such as public-holiday lookup, local-time conversion, work-window evaluation, exception evidence, or preference scoring, before composing them into higher-level reachability APIs.
